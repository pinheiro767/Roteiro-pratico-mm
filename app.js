...
function criarCard(item, regiao, idx){
  ...
  input.addEventListener("change", (e) => salvarFotos(e, id, nome));

  setTimeout(() => renderFotos(id), 0);
  return card;
}

function salvarFotos(event, id, nome){
  const files = Array.from(event.target.files || []);
  const fotosAtuais = JSON.parse(localStorage.getItem("fotos_" + id) || "[]");

  files.forEach(file => {
    const reader = new FileReader();
    reader.onload = () => {
      fotosAtuais.push({nome:file.name || nome, data:reader.result, dataHora:new Date().toLocaleString("pt-BR")});
      localStorage.setItem("fotos_" + id, JSON.stringify(fotosAtuais));
      renderFotos(id);
    };
...
function renderFotos(id){
  ...
  if(!box) return;
  const fotos = JSON.parse(localStorage.getItem("fotos_" + id) || "[]");
  box.innerHTML = "";
  fotos.forEach((foto, i) => {
    const fig = document.createElement("figure");
    fig.innerHTML = `
      <img src="${foto.data}" alt="Foto ${i+1}">
      <figcaption>Foto ${i+1}<br>${foto.dataHora || ""}</figcaption>
      <button type="button" onclick="removerFoto('${id}',${i})">Remover</button>
    `;
    box.appendChild(fig);
  });
}
...
function removerFoto(id, index){
  ...
  localStorage.setItem("fotos_" + id, JSON.stringify(fotos));
  renderFotos(id);
}
